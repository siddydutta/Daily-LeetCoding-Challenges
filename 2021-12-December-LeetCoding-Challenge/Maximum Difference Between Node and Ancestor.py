...
class Solution:
    def maxAncestorDiff(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node: TreeNode, high: int, low: int) -> int:
            ''' Returns the maximum difference in that subtree. '''
            if node is None:
                return high-low  # Compute diff of subtree at leaf node
            # Update high and low values for the subtree
            high, low = max(high, node.val), min(low, node.val)
            # Return the maximum difference from both descendants
