...
class RandomizedSet:
    ...
    def remove(self, val: int) -> bool:
        if val in self.index:
            # Replace index of val with last number in array
            idx = self.index[val]
            last_val = self.numbers[self.length-1]
            self.numbers[idx] = last_val
            self.index[last_val] = idx  # Map new index to last val
...
def main():
    ...
    randomizedSet = RandomizedSet()
    assert not randomizedSet.remove(0)
    assert not randomizedSet.remove(0)
    assert randomizedSet.insert(0)
    assert randomizedSet.getRandom() == 0
    assert randomizedSet.remove(0)
    assert randomizedSet.insert(0)
